    ...
    public function register(Container $pimple)
    {
        $pimple['routerFile'] = function (Container $container) {
            if (APPLICATION_ENV === 'development' || !file_exists(APP_PATH . '/Routers/router.lock')) {
                if (file_exists($container['application']->config('app.router_cache_file' , sys_get_temp_dir().'/'.strtolower(APP_NAME).'routerCacheFile.router'))) {
                    @unlink($container['application']->config('app.router_cache_file',sys_get_temp_dir().'/'.strtolower(APP_NAME).'routerCacheFile.router'));
                }
                ...
                $container['router']->setCacheFile($container['application']->config('app.router_cache_file', sys_get_temp_dir().'/'.strtolower(APP_NAME).'routerCacheFile.router'));
